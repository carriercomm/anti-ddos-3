...
public class SomeServer implements Agent{
	...
	timer.scheduleAtFixedRate(new TimerTask() {
  		...
  		public void run() {
			...
			{	FileWriter writer = new FileWriter("packet_counter.csv",true);
				writer.append("LGI");
	    			writer.append(',');
	    			writer.append("" + (SomeServer.counter - SomeServer.last));
	    			writer.append('\n');
				writer.flush();
	    			writer.close();
			}
			...
			SomeServer.last = SomeServer.counter;
		}
	}, 1000, 1000);
	...
	SomeServer somea = new SomeServer();
	somea.run(args);
    }
